...
public partial class AliyunDriveApiClient
{
    ...
    public async Task<T> SendJsonGetAsync<T>(string url, bool prepareToken = true)
    {
        if (url == null)
            throw new ArgumentNullException(nameof(url));
    ...
    private async Task<T> SendJsonPostNoTokenAsync<T>(string url, SendRequestFrom obj, bool prepareToken = true)
    {
        if (url == null)
            throw new ArgumentNullException(nameof(url));
        ...
        var content = new StringContent(obj.BuildParams(), Encoding.UTF8, "application/x-www-form-urlencoded");
        var resp = await _httpClient.PostAsync(url, content);
        var json = await TryThrowExceptionAndReadContentAsync(url, resp);
        return JsonSerializer.Deserialize<T>(json, JsonSerializerOptions);
    }
